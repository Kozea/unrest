...
class AuthDecoratorsTestCollection(object):
    def test_auth_decorator(self):
        ...
        assert self.fetch('/api/tree/1')[0] == 200
        assert self.fetch('/api/tree/2')[0] == 403
        assert (
            self.fetch(
        ...
        assert (
            self.fetch('/api/tree/1', method="POST", json={'name': 'cedar'})[0]
            == 501
        )
        assert (
            self.fetch('/api/tree/2', method="POST", json={'name': 'cedar'})[0]
            == 403
        )
        assert self.fetch('/api/tree/1', method="DELETE")[0] == 200
        assert self.fetch('/api/tree/2', method="DELETE")[0] == 403

    def test_read_write_decorator(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
        ...
        assert self.fetch('/api/tree/1')[0] == 403
        assert self.fetch('/api/tree/2')[0] == 200
        assert (
            self.fetch(
                '/api/tree/1', method="PUT", json={'id': 1, 'name': 'cedar'}
            )[0]
    ...
    def test_all_decorator(self):
        ...
        assert self.fetch('/api/tree/1')[0] == 403
        assert self.fetch('/api/tree/2')[0] == 200
        assert self.fetch('/api/tree/3')[0] == 403
        assert (
            self.fetch(
                '/api/tree/1', method="PUT", json={'id': 1, 'name': 'cedar'}
            )[0]
        ...
        assert (
            self.fetch('/api/tree/3', method="POST", json={'name': 'cedar'})[0]
            == 403
        )
        assert self.fetch('/api/tree/1', method="DELETE")[0] == 200
        assert self.fetch('/api/tree/2', method="DELETE")[0] == 403
        assert self.fetch('/api/tree/3', method="DELETE")[0] == 403
