...
class PropertiesTestCollection(object):
    def test_get_tree_with_property(self):
    ...
    def test_get_tree_with_object_property(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
    ...
    def test_get_fruit_with_hybrid_property_and_type(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
    ...
    def test_get_fruit_with_hybrid_property_and_formatter(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
        rest(
            Fruit,
            properties=[
    ...
    def test_get_fruit_with_primary_keys(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
    ...
    def test_get_tree_with_hybrid_property_as_primary_key(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
        rest(
            Fruit,
    ...
    def test_get_tree_pk_with_hybrid_property_as_primary_key(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
        rest(
            Fruit,
            primary_keys=['square_size'],
        ...
        code, json = self.fetch('/api/fruit/0.25')
        assert code == 200
    ...
    def test_datetime_properties(self):
        ...
        rest(
            Fruit,
            only=[],
            properties=[rest.Property('birthday', type=DateTime())],
        )
        code, json = self.fetch('/api/fruit')
        assert code == 200
        assert json['occurences'] == 5
