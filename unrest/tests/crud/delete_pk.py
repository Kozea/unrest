...
class DeletePkTestCollection(object):
    ...
    def test_delete_unknown_tree(self):
        ...
        code, json = self.fetch('/api/tree/9', method="DELETE")
        assert code == 404

    def test_delete_fruit(self):
        rest = UnRest(self.app, self.session, framework=self.__framework__)
        rest(Fruit, methods=['GET', 'DELETE'])
        code, json = self.fetch('/api/fruit/2', method="DELETE")
        assert code == 200
        assert json['occurences'] == 1
